...
SOURCE_FILES = (
    ...
    "mixllm/kernels/cutlass_extension/mq_mma_tensor_op_sm75.h",
    "mixllm/kernels/cutlass_extension/mq_mma_mixed_input_tensor_op.h",
...
if capability == (7, 5):
    ...
    ref_rows = torch.arange(8, dtype=torch.int32)[:, None]
    ref_k_a = torch.arange(32, dtype=torch.int32)[None, :]
    ref_k_b = torch.arange(32, dtype=torch.int32)[:, None]
    ref_cols = torch.arange(8, dtype=torch.int32)[None, :]
    low_ref = ((ref_rows * 3 + ref_k_a * 5 + 1) & 15)
    high_raw_ref = ((ref_rows * 5 + ref_k_a * 3 + 7) & 15)
    high_ref = torch.where(high_raw_ref >= 8, high_raw_ref - 16, high_raw_ref)
    b_ref = ((ref_k_b * 7 + ref_cols * 3 + 2) & 15)
    nonuniform_ref = ((low_ref + 16 * high_ref).to(torch.int64) @ b_ref.to(torch.int64)).to(torch.int32).reshape(1, 64).to('cuda')
    ...
    crosswise_native_store_probe = torch.ops.mixllm_sm75.sm75_int4_pair_crosswise_u16_native_store_probe(torch.empty(0, device='cuda'))
    ...
    assert torch.equal(mn_native_store_probe, mn_reference), (mn_native_store_probe, mn_reference)
    print('SM75_INT4_CROSSWISE_U16_MN_OFFSET_NATIVE_STORE_REFERENCE_PASS', mn_native_store_probe[:, :2, :4].tolist(), flush=True)
