...
LOG = Path('/tmp/kaggle-v254-final.log')
...
Path('/tmp/kaggle-v254-stdout.txt').write_text(stdout)
...
for marker in [
    'SM75_INT4_PAIR_MIXED_STRIDE_STATS',
    'SM75_INT4_PAIR_MIXED_STRIDE_MISMATCH_COUNT',
    'SM75_INT4_PAIR_MIXED_STRIDE_PROBE_PASS',
    'TESTS_RETURNCODE',
    'GATES_PRE_EXEC',
    'BENCHMARKS_PRE_EXEC',
]:
...
payload = stdout[start:end if end >= 0 else len(stdout)].strip()
report = ast.literal_eval(payload)
for name, scenario in report.get('scenarios', {}).items():
    if 'qwen_qkv_mixed' in name:
