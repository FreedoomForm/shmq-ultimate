...
## Packed-storage bridge

The next proof bridges the maintained model storage contract rather than writing U16 logical values directly. Production `_pack_uint4` places logical even K in a byte’s low nibble and odd K in its high nibble; the proof will fill a linear packed-byte weight buffer for 16 output channels and K=32, unpack each byte by parity, and place the resulting U4 code into the physical Crosswise U16 B layout. It will also fill an int8 activation buffer, derive low=`activation & 15` and signed high=`activation >> 4` with the established exact decomposition, and place those planes into the physical Crosswise U16 A layouts. Four co-resident warps will then use the validated native loader/MMA/store path for the same M/N tiles. The reference checks the raw native-code product first; zero-point correction remains a separate seam and will not be silently folded into this bridge proof.

This isolates byte-nibble order and the production activation decomposition from the physical Crosswise contract. The packed buffer is deliberately nonuniform and includes odd/even K values, while the output remains research-only and no production dispatch is changed.
