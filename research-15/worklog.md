...
## v296 — restore original widened packed A iterator shape — 2026-08-22

- Deep research compared the original MixLLM `MQMmaMixedInputTensorOp` against SHMQ’s SM75 `MQMmaPackedInputTensorOpSm75`. The original widened INT8 A iterator is instantiated with the original operator’s `MatrixShape<16,32>` contract, while SHMQ had substituted the legal SM75 instruction height `<8,32>`. Although both fragments have the same total register count, the row-major crosswise iterator’s lane/LDSM mapping depends on the iterator shape. The SM75 adapter still uses the legal `m8n8k16` arithmetic and two-half accumulation; only the A iterator shape is restored to `<16,32>`.
- Updated the stale v275 source contract to require `<16,32>` and added the v296 contract rejecting the old `<ArchMmaOperator::Shape::kM,32>` form. Local targeted suite now reports `94 passed, 6 skipped, 2 CUDA-only failures` because this sandbox’s PyTorch has no CUDA; the prior full-suite collection failure from vendored CUTLASS examples is not a project test failure.
- This is not yet an accepted baseline. The committed candidate must compile and run the operator notebook on a Tesla T4 / SM75 through Colab. Keep only if large-M packed INT4 correctness, timing integrity, and all required operator gates pass; no Kaggle run is launched during repair.


## v296 Colab operator benchmark — 2026-08-22 — NO-GO

- The source-verified v296 operator notebook compiled and executed on Tesla T4 / SM75 through the official Colab CLI. Embedded contracts passed, timing integrity was true for all measured shapes, and the uploaded notebook manifest matched the current v296 tree exactly. Full-model Qwen quality was intentionally not run in operator-only mode.
- Restoring the A iterator from `<8,32>` to the original widened `<16,32>` contract did not repair large-M packed INT4 correctness. Qwen-shaped mixed max error remained `818.567` at rows=128 and pure INT4 max error remained `877.446` at rows=128; smoke mixed rows=32/128 errors were `212.165`/`266.186`. Therefore `sm75_native_correctness` and mixed prefill failed, and v296 is rejected as a production candidate; v271 remains the last Kaggle-confirmed safe baseline.
- Representative Qwen-shaped mixed speedups versus dense FP16 were `0.8388x` at rows=1, `0.2870x` at rows=16, and `0.4862x` at rows=128. These values are not accepted as performance claims because correctness failed. The measured packed storage remained `9.84 MB` versus `25.69 MB` dense weights, while warm INT4 expansion at rows>1 remained `8.60 MB`.
- Deep follow-up comparison identified the next isolated discrepancy: CUTLASS’s generic SM75 path sets `kVerticalVisit=true`, uses `n`-outer / `m`-inner serpentine visitation, and performs no A/B register shuffle in its vertical transform. SHMQ’s custom adapter still uses the SM80 nonvertical ordering and applies the A shuffle. v297 will port only this SM75 vertical visitation/transform contract while retaining the two legal k16 MMAs and all benchmark conditions.


## v297 — port CUTLASS SM75 vertical visitation to packed adapter — 2026-08-22

- v296 compiled on T4 but reproduced the v295 large-M corruption, proving that the widened A iterator alone was not the root cause. Deep comparison of original CUTLASS `mma_tensor_op.h` against SHMQ’s custom adapter found that CUTLASS sets `kVerticalVisit=true` for every architecture below SM80. Its SM75 path visits `n` outer / `m` inner with `m_serpentine`, uses the vertical accumulator slot formula, and preserves the loaded A fragment in `transform()`; SHMQ still used the SM80 nonvertical order and an extra A shuffle.
- v297 changes only the packed SM75 adapter’s logical MMA visitation and A transform to match that documented SM75 contract. The two legal `m8n8k16` calls per widened k32 fragment, B no-shuffle behavior, `<16,32>` iterator, packed layout, metadata, streams, model, quality, and benchmark settings remain unchanged. Source contracts were updated to reject the old A-shuffle assumption and assert vertical visitation; the focused SM75 source suite reports `34 passed`.
- The candidate is pending a fresh source-verified Colab T4 operator run. It will be retained only if large-M packed INT4 correctness and timing integrity pass together with the existing operator checks. Kaggle remains prohibited during this repair iteration.


## v297 first Colab attempt invalidated — 2026-08-22

The first v297 execution used the same persistent Colab Python kernel as v296. Because `sm75_backend.load_sm75_backend()` returns immediately when its module-level `_LOADED` flag is already true, the v297 notebook reused the v296-compiled extension whose CUDA translation unit name and digest were unchanged. The v297 notebook itself had the correct source manifest, but its numerical result cannot validate the changed custom header. The apparent unchanged errors and timings are therefore classified as an environment/cache artifact, not a v297 benchmark result. The session must be torn down and v297 rerun in a fresh T4 process before any accept/reject decision.


## v297 fresh-process Colab result — 2026-08-22 — NO-GO

- A fresh Tesla T4 Colab session compiled the v297 custom header, eliminating the v296 extension-cache artifact. The embedded source contracts and execution completed, and mixed decode performance passed. However, SM75 native correctness still failed: Qwen mixed rows=128 remained `max_abs_error=818.567`, pure INT4 rows=128 remained `877.446`, and smoke mixed rows=32/128 remained `212.165`/`266.186`. The generic SM75 vertical visitation port therefore does not fix the packed v3 corruption.
- Timing integrity failed only because the candidate’s large-M runs had unstable ratios in this fresh session; regardless, the correctness failure is decisive. The candidate remains rejected and v271 remains the last safe baseline. The unchanged large-M errors, while rows<32 continue to use the legacy expanded path and pass, localize the defect to the native packed INT4 v3 path rather than the general three-level arithmetic.
- The next research seam is the original global packed-B contract and its threadblock-to-warp handoff. The two-stage host interleave matches the original helper, but the custom SM75 adapter still assumes that converting the loaded `uint4b_t` fragment directly to 64 signed bytes and splitting it at `MmaIterations::kColumn` reproduces the original k32 register grouping. This must be proven from the original `mma_multistage` runner and CUTLASS iterator/fragment layout before another code change.


## v298 — packed B fragment register-order repair (pre-Colab)

Deep research compared the original MixLLM `m16n8k32` path with CUTLASS’s SM75 iterator implementation. The active packed adapter loads a 32-value `uint4b_t` B fragment for a 32x32 warp tile and expands it to 32 signed bytes, then treats the eight `m8n8k16` B operands as `[all four N tiles for the first K half][all four N tiles for the second K half]` by using `ptr_B[n]` and `ptr_B[4+n]`. CUTLASS’s iterator/load contract instead emits one packed 32-bit register per N tile; each register expands to eight values containing both K halves for that N tile. The faithful split is therefore n-major: `ptr_B[2*n]` and `ptr_B[2*n+1]`. This is an isolated register-grouping repair derived from the iterator’s `FragmentB` width and `NumericArrayConverter<int8_t,uint4b_t,8>` specialization. It leaves the original interleaved weight bytes, metadata, arithmetic, output scatter, benchmark settings, and stream topology unchanged. v297 remains rejected pending this test.
...
## v302 Kaggle validation — version 276 (failed compile; stale CUTLASS vendor bundle)

The v302 notebook was submitted from the synchronized Windows checkout as Kaggle kernel version 276. The T4 run reached CUDA compilation and failed before executing gates. The compiler diagnostic was: `mq_mma_pipelined_sm75.h(648): error: class cutlass::gemm::warp::MmaTensorOp<...> has no member kSkipKgroupIndex`, with the same error at the B setter site. The local v302 source adds the default trait to vendored `cutlass/include/cutlass/gemm/warp/mma_tensor_op.h`, but the authoritative notebook embeds `mixllm/kernels/cutlass_sm75_vendor.b64`, and that bundle was not regenerated after the header change. The downloaded Kaggle copy of `mma_tensor_op.h` confirms the trait is absent. This is an artifact-provenance failure, not evidence against the v302 kernel logic; no performance or correctness gate ran. The next repair is to regenerate the CUTLASS vendor archive from the committed v302 source, rebuild the notebook manifest/digest, rerun all local source checks, commit the artifact, and only then submit a fresh Kaggle run.

## v302 corrected artifact — Kaggle version 277 (compiled; correctness rejected)

After regenerating `cutlass_sm75_vendor.b64` and rebuilding the authoritative notebook, the corrected v302 notebook compiled successfully on the NVIDIA T4. Embedded contract tests passed (`99 tests`, `OK (skipped=2)`), and the SM75 instruction/load/fused probes passed. The run nevertheless failed the native correctness and mixed-prefill end-to-end gates, so the candidate is rejected and no performance claim is accepted.

The failure is shape-dependent. On the Qwen QKV mixed 4/8/16 scenario, rows 1 and 16 remained numerically close (`max_abs_error` 0.125), but rows 128 produced `max_abs_error=731.8734`; pure INT4 rows 128 produced `max_abs_error=772.9107`. Mixed decode GEMM and mixed decode end-to-end gates passed, while mixed prefill end-to-end failed. At rows 128 the mixed prefill p50 was 0.507696 ms versus dense FP16 0.246096 ms (`0.484731x` speedup, i.e. slower), and pure INT4 was 0.305152 ms versus 0.178208 ms (`0.583997x`). Timing integrity passed. The corrected vendor artifact therefore fixed the compile-provenance error but did not fix the underlying large-M packed-native correctness bug. Next iteration must perform fresh original-versus-SHMQ deep research before any code change; current confirmed safe baseline remains v271.

## v284 — Kaggle version 278 result (compile failure; rejected)

The exact v284 source commit was `ccd74338f0bda0c759a523036cc0cc5d17acb103`, and the connected Windows checkout was pinned to that commit before submission. Kaggle accepted it as kernel version 278 and allocated the T4, but CUDA compilation failed before any correctness or performance gate ran. The compiler stopped at `cutlass/include/cutlass/transform/pitch_linear_thread_map.h(298)` with `static assertion failed with "Number of iterations must be non-zero"`; the kernel ended with `RuntimeError: Error building extension 'mixllm_sm75_backend_94472f11e17c58af'`. Therefore v284 has no admissible T4 metrics and is rejected for this run. The previously recorded v284 Colab attempt was also blocked before execution by the Colab assignment limit. The Windows checkout remains detached at v284 for provenance; restore it to the current development branch before further work.

## Iterative repair after v284 — selected v286 compile-safe shape fix

Deep research against original MixLLM and vendored CUTLASS confirms that v284’s failure occurs before the custom packed adapter: `DefaultMmaCore` instantiation for the packed `M128/N64` alias drives `PitchLinearWarpRakedThreadMap::Iterations::kCount` to zero for `uint4b_t`. Original MixLLM does not solve this by changing arithmetic or quality settings; it relies on valid CUTLASS shape families selected by the launcher. The smallest safe repair is therefore the previously audited v286 change: remove only the packed `M128/N64` alias and its `rows>=96` dispatch, route all native packed INT4 through the compile-safe `M64/N64` family, and advance the tuning ABI to invalidate incompatible cached shape entries. Ordinary INT8 `M128/N64`, the three-level scheduler, persistent streams, metadata, scatter ABI, precision, model, benchmark thresholds, and quality checks remain unchanged. This repair will be locally checked before any new Kaggle submission.

## Maximal-safe v284-v291 + v299 fallback — Kaggle version 280 result

The combined candidate at commit `0c1a54c2ae1a25432b96fd14505ff0b5b040f74d` compiled and executed successfully on Kaggle T4. Embedded tests, SM75 native probes, fixed/automatic allocator checks, native correctness, both decode gates, and timing integrity passed. Native correctness remained within tolerance at large M because the unsafe packed v3 dispatch was disabled and the original expanded INT4 path was used.

The production decision was nevertheless `no_go`: mixed prefill end-to-end performance failed, full-model Qwen quality/throughput were `unavailable_environment`, and therefore `operator_production` and `model_vllm_production` failed. Representative Qwen-shaped mixed end-to-end speedups versus dense FP16 were approximately 0.968x at rows=1, 0.385x at rows=16, and 0.067x at rows=128; pure INT4 rows=128 was approximately 0.067x. Timing integrity passed, and Qwen-shaped mixed rows=128 numerical correctness was `max_abs_error=0.1239624` versus the quantized reference and `2.125` versus dense FP16. This confirms the maximal-safe set restores correctness but the expanded INT4 fallback is not a performance solution. The candidate is retained as a correctness control only; no new performance baseline is established. The next iteration must research a new native packed design rather than re-enable the rejected v294–v302 fragment path.

## Next native design research after maximal-safe v299 fallback

The maximal-safe candidate restored numerical correctness but exposed the remaining bottleneck: expanded INT4 prefill is approximately 15x slower than dense FP16 at Qwen-shaped rows=128, despite passing timing-integrity and correctness. Fresh comparison with original MixLLM shows the upstream packed path uses `matrix_B_interleaved` directly with a legal mixed-input operator policy and a single `gemm_k_iterations` loop. Its SM80 integer specialization uses `GemmShape<16,8,32>` and the generic mixed-input operator, while SHMQ SM75 wraps m8n8k16 in a synthetic k32 policy, widens fragments, and manually calls two MMAs. The repeated v294–v302 large-M failures are therefore attributed to the synthetic policy/iterator contract rather than one missing fragment permutation.

The next implementation will be isolated from production dispatch and will use a self-consistent legal SM75 k16 policy or an explicit loader/MMA kernel. The expanded v299 path remains the correctness fallback until the new native path passes independent small/large-M probes. No benchmark settings, quality thresholds, model, or quantization allocation will change.

## Next candidate: reuse proven fused packed INT4 pair inside mixed prefill

The maximal-safe v299 run confirms expanded INT4 is the dominant remaining cost. The existing `run_int4_pair_partition` kernel is already used for pure INT4 rows>=32 and its source contract proves direct packed-nibble loading, zero-point handling, activation scaling, and indexed output writes. Its signature writes only the INT4 partition’s indexed columns and does not depend on n8/n16, so it can be launched concurrently with the existing INT8 stream and caller-stream FP16 partition for mixed output. The current restriction to pure INT4 is documented as a performance heuristic, not a correctness requirement.

The next candidate will use the proven fused packed pair for mixed rows>=32, allow the expanded INT4 ABI tensor to remain empty when the packed tensor is present and the fused branch is selected, and retain the expanded v2 path for rows<32 or unavailable packed data. This removes the expanded INT4 materialization from the mixed large-M hot path without changing quantization, output mapping, stream/event ordering, or benchmark settings. The candidate remains behind existing shape and packed-data checks and will be rejected if any mixed correctness or timing gate fails.

## Mixed packed-INT4 fused candidate — Kaggle version 281 result

The candidate compiled and passed embedded contracts, SM75 probes, native correctness, decode gates, and allocator gates. The mixed fused INT4 branch was numerically correct, but the benchmark still reported `expanded_int4_bytes=12845056` for Qwen-shaped rows 16/128 and `32768` for smoke rows 32/128. The backend still materialized `_expanded_int4_for_prefill()` before calling the v2 operator because v299’s safe dispatch always requested the legacy fallback tensor. The C++ fused branch then ignored that tensor, so the candidate removed the native packed adapter but did not remove its expansion cost.

The mixed Qwen rows=128 end-to-end speedup remained approximately `0.0661x` versus dense FP16, and timing integrity failed in this run. The next safe repair is to pass the existing empty int8 placeholder from Python for rows>=32 with an INT4 partition, exactly when the v2 C++ dispatcher selects the proven fused packed pair (`!has_cached_metadata`), while retaining expansion for rows<32 and all cached/legacy fallback cases. This changes no arithmetic or benchmark settings and directly addresses the measured redundant allocation.

## Next candidate: skip redundant expansion when mixed packed pair is selected

The v281 result showed the new C++ mixed fused branch was correct but Python still allocated the full signed INT4 expansion before the call. This candidate changes `three_level_linear_prequantized()` so rows>=32 with an INT4 partition pass the existing empty int8 placeholder; the C++ validator accepts that placeholder only when `use_fused_int4` is true, and the unified scheduler selects `run_int4_pair_partition` on the integer stream. Rows<32 retain the expanded path, and cached-metadata/legacy callers retain their existing validation. Local suite: `88 passed, 6 skipped, 3 subtests passed`; compileall and diff checks pass. Kaggle T4 validation is required for correctness, timing integrity, memory accounting, and performance.

## Mixed packed-INT4 fused candidate — Kaggle version 282 result

Version 282 compiled and passed embedded tests, SM75 probes, native correctness, allocator checks, and timing integrity. The mixed fused pair launch did not improve the production gates: mixed decode end-to-end performance failed, mixed prefill end-to-end performance failed, and the production decision was `no_go`. Qwen-shaped mixed rows=128 end-to-end speedup was approximately `0.0672x` versus dense FP16; smoke mixed rows=128 was approximately `0.1180x`. Full-model Qwen quality and vLLM apply remained unavailable in the environment. Reported `expanded_int4_bytes` was still nonzero for larger cases because the benchmark/reference path materialized the module’s cached expansion for telemetry, so the change did not establish a valid memory or performance improvement. The fused mixed branch is rejected and will be removed from the production candidate; the previous maximal-safe expanded fallback remains the correctness control.

## Candidate: larger-tile fused INT4 pair kernel

After rejecting the mixed fused dispatch, stage timing isolated the slow path to the existing fused INT4 kernel itself: Qwen pure INT4 rows=128 took about 2.31 ms versus dense FP16 about 0.156 ms. The source and NVIDIA Turing documentation show that the current kernel’s 32-row x 64-channel CTA is much smaller than the original MixLLM/CUTLASS tile families. This candidate keeps the verified arithmetic and output mapping but templates the fused pair kernel, launches an 8-warp/128-channel variant for channels>=128, and retains the existing 4-warp/64-channel variant below 128. The larger variant uses the same 32-row tile and 4 row subtiles, with an explicit `warp < 4` guard for row sums; grid X changes from the old 32-channel stride to the actual 128/64 tile capacity to remove overlaunch.

Local validation after the change: `86 passed, 6 skipped, 3 subtests passed`; Python compileall and diff checks pass. The candidate requires a T4 run to prove the larger shared-memory/register footprint does not harm correctness, timing integrity, or occupancy.

## v283 — larger fused INT4 tile candidate (commit 5663f095e8a2341b8aa1db052949b9047cb3aec6)

Version 283 was submitted only after the local suite, compileall, diff checks, authoritative notebook rebuild/check, and source-provenance validation passed. The experiment confirmed that the larger 8-warp/128-channel fused INT4 pair kernel is numerically safe in its explicit probes: `SM75_INT4_PAIR_INSTRUCTION_PROBE_PASS`, `SM75_INT4_PAIR_WMMA_LOAD_PROBE_PASS`, `SM75_INT4_PAIR_FUSED_PROBE_PASS`, and the mixed-stride probe all passed with zero mismatches. T4 hardware, native correctness, allocator, timing-integrity, and embedded-contract gates passed.

However, this was not a live production performance candidate. The current production route still calls `run_unified_prefill(..., false)`, and Python keeps `native_v3=None` and `use_cached_v3=False`; therefore normal rows>=32 inference cannot reach `run_int4_pair_partition()`. The v283 timings consequently measured the unchanged expanded-INT4 fallback: Qwen mixed 4/8/16 E2E speedups versus dense FP16 were 1.009x at rows=1, 0.322x at rows=16, and 0.492x at rows=128; pure INT4 was 0.968x, 0.604x, and 0.646x; pure INT8 was 0.950x, 0.550x, and 0.781x; pure FP16 was 0.951x, 0.360x, and 0.143x. Mixed prefill E2E performance failed, model/vLLM production failed, and full-model Qwen quality/throughput plus vLLM apply were unavailable in the Kaggle environment. Terminal decision: `no_go`.

Interpretation: v283 validates only the isolated kernel/probe contract and does not improve the production baseline. Do not claim a speedup or retain it as a performance result. The next iteration must begin with primary-source comparison of the original MixLLM launcher/config/cache/packed-layout architecture, official legal Turing MMA/layout/pipeline constraints, and the reachable SHMQ dispatcher seam. Any new dispatch must be narrow, guarded, single-writer, correctness-probed on T4, and retain the expanded fallback for all unsupported or failed shapes.

## v284 — guarded live larger-tile packed INT4 dispatch (pending T4)

The v283 deep research compared the original MixLLM launcher and interleave contract with official CUTLASS Turing documentation and the SHMQ implementation. The compatible seam is the existing persistent integer fork/join, not a new serialized scheduler: keep the original packed `matrix_B_interleaved` tensor, use a real SM75 `m8n8k32` pair operation only where complete aligned tiles are guaranteed, and preserve the staged expanded-INT4 fallback everywhere else.

The v284 edit set makes the already-probed larger pair kernel reachable only when `rows >= 32`, `rows % 32 == 0`, `width >= 128`, `width % 128 == 0`, and the INT4 partition has at least 128 channels. Python then avoids the redundant signed expanded cache only for that exact guard. C++ uses the same guard to allow the empty expansion ABI slot and passes `use_fused_int4` through the existing persistent-stream scheduler; all unsupported/tail/small-M shapes retain the expanded CUTLASS path. No arithmetic, model, partition allocation, thresholds, benchmark settings, or quality criteria changed. New unit/source-contract tests cover the guard and dispatch seam.

Local validation: targeted backend/source tests passed (`46 passed, 6 skipped, 7 subtests`); intended project tests passed (`93 passed, 6 skipped, 7 subtests`) with only two CUDA-only vendored integration tests excluded because the sandbox PyTorch has no CUDA; compileall passed. This candidate is not accepted until T4 proves native correctness, single-writer behavior, timing integrity, allocator/memory safety, mixed prefill/decode performance, full-model Qwen quality/throughput, and vLLM production gates.

## v284 result — guarded live packed INT4 path rejected

Kaggle T4 version 284 compiled and exercised the guarded live dispatch. The explicit INT4 instruction, WMMA-load, fused, and mixed-stride probes passed, with zero mixed-stride mismatches; native correctness, allocator checks, timing integrity, T4 hardware, and embedded tests passed. This establishes that the tested isolated arithmetic is not corrupt under the probe inputs.

The production result was a clear no-go. Qwen mixed 4/8/16 E2E speedup versus the unchanged dense FP16 baseline was 0.981x at rows=1, 0.286x at rows=16, and 0.166x at rows=128. Pure INT4 was 0.958x, 0.594x, and 0.163x; pure INT8 was 0.944x, 0.599x, and 0.730x. The mixed prefill E2E gate failed, operator production failed, model production failed, and full-model Qwen quality/throughput plus vLLM apply remained unavailable. One pure-INT4 rows=16 timing-integrity sample also failed, although the aggregate timing gate passed. The terminal decision was `no_go`.

Because the live packed pair path did not improve performance and was materially slower at large M, it is not retained in production. The v284 source dispatch and its Python expansion bypass are reverted immediately; the v299 expanded INT4 route remains the correctness control. The v284 log is retained as evidence, not as a speedup claim. The next iteration must research and implement a genuinely CUTLASS-style native packed SM75 dataflow rather than expose this small direct pair kernel.

## v285 — B-fragment reuse inside the guarded pair candidate (pending T4)

The v284 primary-source audit found that official CUTLASS expresses warp-level reuse by traversing warp-K, warp-N, then warp-M tiles, while the direct SHMQ pair kernel loaded and converted the same B fragment once per row tile. The v285 change hoists each warp’s two cached B fragments outside the four-row-subtile loop and reuses them for all row tiles. It retains the legal SM75 `m8n8k32` low/high nibble arithmetic, packed interleave, scales, zero correction, output scatter, persistent stream/event overlap, and the v284 aligned guard. The expanded INT4 CUTLASS fallback remains selected for every unsupported shape.

Targeted local validation passed: `46 passed, 6 skipped, 7 subtests passed`. The candidate remains pending complete project validation, authoritative notebook rebuild, commit/push, and a new T4 gate; no performance claim is accepted.

## v285 result — B-fragment reuse rejected

Kaggle T4 version 285 compiled and all explicit INT4 probes, native correctness, allocator, timing-integrity aggregate, T4 hardware, and embedded tests passed. The B-reuse edit was numerically safe in the tested probes, but it did not make the pair path competitive: Qwen mixed 4/8/16 E2E speedup versus dense FP16 was 1.039x at rows=1, 0.283x at rows=16, and 0.185x at rows=128; pure INT4 was 0.964x, 0.539x, and 0.166x. Mixed prefill E2E failed, operator/model production failed, and full-model Qwen quality/throughput plus vLLM apply remained unavailable. Terminal decision: `no_go`.

The improvement from v284 to v285 at Qwen rows=128 was small and remained far below the 2.6x target; the pair kernel’s dominant cost is therefore not only repeated B fragment loads. The v285 production exposure is reverted immediately, retaining the v284 research and raw log as evidence and restoring the expanded INT4 correctness control. The next iteration must target the actual staged CUTLASS dataflow or a mathematically fused alternative rather than continue optimizing the rejected direct pair kernel.

## v286 — four-warp 128-channel pair ownership (pending T4)

Deep research compared the official CUTLASS warp-N/warp-M hierarchy and Microsoft MixLLM’s configurable warp shape against the v284/v285 direct pair kernel. The large pair candidate assigned eight warps to 16-channel strips, causing eight redundant A-fragment loads for the same 32-row activation tile. v285 only hoisted B within each warp and improved Qwen mixed rows=128 from 0.166x to 0.185x, so repeated B loads were not the dominant cost.

The v286 candidate changes only pair-kernel ownership: four warps cover a 128-channel tile, each warp handling four 8-channel native subtiles; the existing 64-channel family remains four warps handling two subtiles. Arithmetic, packed layout, zero correction, scales, output mapping, streams, benchmark settings, and expanded fallback are unchanged. This is pending local validation and one authoritative T4 gate; no performance claim is made.

## v286 result — four-warp 128-channel ownership rejected
...
## v288 — legal stage-5 runner around the existing SM75 core (pending T4)

Fresh primary-source research found that Microsoft MixLLM supplies stage count separately from its legal GEMM core, while the previous SHMQ v222 failure came from requesting the unsupported `DefaultMmaCore<...,5>` specialization. v288 instead instantiates the existing legal SM75 `Core` through `Runner<Core,5>`, preserving `32x128x64 / 32x32x64 / 8x8x16`, synchronous SM75 staging, exact expanded INT4/INT8 arithmetic, separate auxiliary streams, and all fallback behavior. The stage-5 candidate is limited to rows>=128 and remains inside the existing tuner. NVIDIA’s documented occupancy trade-off makes this an experiment, not an assumption. Pending complete local validation and one T4 gate; no performance claim is made.

## v288 result — stage-5 runner rejected by timing and production gates

Kaggle T4 v288 compiled and passed explicit probes, allocator checks, and the embedded test suite. The legal `Runner<Core,5>` improved the selected large-M expanded path relative to the prior control in some isolated measurements: pure INT4 rows=128 reached 0.703x E2E versus dense and pure INT8 reached 0.717x. It was still slower than FP16, far below the 2.6x target, and the mixed Qwen path remained 0.291x at rows=16 and 0.333x at rows=128. The mixed rows=16 timing-integrity check failed; smoke mixed rows=128 also exhibited a large correctness error (`48.79`) because the guarded stage-5 selection affected the small smoke partition despite its production-shape guard. Gates: native correctness failed, mixed prefill failed, timing integrity failed, operator production failed, and full-model Qwen/vLLM gates remained unavailable. Terminal decision: `no_go`.

The stage-5 wrapper is therefore not retained. The experiment confirms that an additional synchronous stage is not a safe drop-in optimization for the current custom SM75 pipeline, and the current tuner/runner seam is too broad for a stage-depth change without a full correctness proof. Restore the validated stage-2 expanded control immediately; keep the research note and raw log as evidence only.

## v289 — restore original interleaved packed handoff through the existing v3 ABI (pending local validation)
...
## v292 — match the original row-major accumulator contract in the packed core (pending local validation)
...
## v294 result — original A-shuffle/B-identity repair did not make the packed SM75 runner correct

v294 was the first run that demonstrably reached the packed path: the mixed and pure-INT4 rows>=32 reports show `expanded_int4_bytes: 0`, while the v299 fallback rows had nonzero expansion. The source-alignment repair therefore received a valid live test. It failed the correctness gate decisively. Smoke mixed rows=32/128 had max absolute errors 212.17/266.48; Qwen mixed rows=128 had 818.57; pure INT4 rows=128 had 877.45. Timing integrity passed, but Qwen mixed rows=128 E2E was only 0.490x and pure INT4 rows=128 was 0.419x versus dense FP16. The packed candidate is no-go and must not be retained. Full-model quality and vLLM production remained unavailable, so no production GO was possible independently of the operator failure.

Conclusion: the original A-shuffle/B-identity transform was a real source mismatch, but it is not the deeper corruption cause. Revert the live v3 enablement, the packed adapter edit, and the packed-placeholder C++ exception. Preserve the v294 research and log as negative evidence. The next seam must be a self-consistent native SM75 m8n8k32 loader/MMA contract, not another permutation-only or dispatch-only change.

## v295 research conclusion — packed SM75 requires a new self-consistent contract

Fresh primary-source review of Microsoft MixLLM, NVIDIA CUTLASS SM75 core/layout code, and NVIDIA Turing documentation confirms that the packed failure spans global layout tags, crosswise shared-memory swizzles, transposed thread maps, warp fragment shapes, instruction ABI, dequantizer correction, and output mapping. CUTLASS’s coherent native INT4 defaults are 128x256x128 threadblock, 64x64x128 warp, 8x8x32 instruction, two stages; SHMQ’s packed route is a plain-layout 32x128x64/32x32x64 custom K32 adapter around two K16 s8*s8 instructions. v294’s live A-shuffle/B-identity test still produced large-M errors up to 877.45, proving that another one-layer permutation edit is unsafe.

An exact arithmetic decomposition of signed INT8 activation into `u4 low + 16*s4 high` is possible in principle, but it would require a new fragment loader/packing ABI and corresponding scale/zero correction proof. It will not be attempted as a local speculative patch. The next implementation seam is either an isolated decomposition/layout probe with full lane and tail coverage, or a source-backed optimization of the safe expanded v299 path; no packed-path performance claim is admissible until such a probe passes.

## v296 result — copied 256x64 family was rejected at CUDA compilation

The v296 notebook was submitted only after local tests, source audit, commit, rebuild, and synchronization. Kaggle failed during nvcc compilation before execution. The diagnostic identifies `CoreM256N64`: its 512-thread block combined with the current plain-layout B thread map for a 64x64 tile and 16 elements per access produces zero pitch-linear iterations. This proves the original configuration tuple is not portable into SHMQ’s fixed runner and is not a performance result. The v296 source was narrowed by removing only `256x64`; the `128x128x64` family remains for a corrected compile-only/performance validation.
...
## v307 research conclusion — measured expanded INT4 bottleneck and explicit warp-iterator seam

Recovered v299 telemetry shows activation quantization is only about 0.026–0.064 ms p50, while larger Qwen expanded INT4 GEMMs take 0.469–2.325 ms and reach 2.72x–14.52x the dense-FP16 GEMM time. The expanded INT4 cache is 8.60 MB for the mixed Qwen layer and 12.85 MB for the pure-INT4 layer. The dominant problem is therefore the expanded INT4 data path/GEMM, not Python bookkeeping or activation quantization. Microsoft’s original stream/event topology is already represented in SHMQ; copying its tile/stage table is unsafe because v288, v299, and v300 already rejected the relevant alternatives.

The vendored CUTLASS source contains explicit warp-level `MmaTensorOpMultiplicandTileIterator` specializations. v307 adds a research-only U4/S4 iterator contract using `TensorOpMultiplicandCongruous<4,64>` and leaves native dispatch disabled. This bypasses the generic threadblock `DefaultMmaCore` family that failed in v304/v305, while reusing the iterator family already present in the manual SM75 adapter. The contract is not a production kernel and does not alter arithmetic, quality, memory gates, or the indexed scatter ABI. Local regression passed: 93 passed, 6 skipped, 3 subtests passed; maintained-package compileall and `git diff --check` passed. A broad historical compileall also reported the pre-existing malformed `research-15/v41_extracted_vllm_three_level.py`, which is unrelated to maintained code.

The v307 commit `5d6e72b` was pushed to `unified-three-level-sm75`. T4 validation could not be launched because the sandbox has no `KAGGLE_API_TOKEN` and the Windows sidecar is currently disconnected. No performance or production claim is made; v299 remains authoritative until this research-only iterator contract is compiled and then extended with lane mapping, tails, scale/zero correction, and scatter proof.

## v309 T4 result — explicit SM75 U4/S4 warp iterators compile successfully

The v309 notebook was pushed from the connected Windows checkout as Kaggle kernel version 307 and completed on NVIDIA T4 / SM75. The prior v308 failure was isolated to the incorrect use of `cutlass::MatrixShape` where CUTLASS’s warp iterator requires pitch-linear shapes. v309 changes only the research-only aliases to `PitchLinearShape<32,32>` for the tile and `PitchLinearShape<32,8>` for the m8n8k32 instruction. The T4 run compiled the full CUDA extension and reported `SM75_INT4_NATIVE_DECOMPOSITION_PROBE_PASS [-480, -480] [8128, 8128]`; embedded tests returned 0, T4 hardware passed, native benchmark/correctness/timing gates passed, and no compiler error remained.

The production decision remains no-go: mixed prefill end-to-end performance failed, full-model Qwen quality/throughput and vLLM apply were unavailable in the gate environment, and native dispatch remains disabled. The v309 artifact measured mixed Qwen rows=128 at 0.746 ms end-to-end (3.09x dense) and pure INT4 rows=128 at 0.305 ms (1.81x dense), but this is not attributable to the research-only aliases and must not be treated as a production speedup. v299 remains the authoritative production baseline. The compile success validates the next seam: a custom block-level loader can now be built on explicit U4/S4 warp iterator fragments rather than the rejected generic `DefaultMmaCore`.

## v316 T4 result — production Crosswise warp iterator probe passes
The v310/v311/v312/v313/v314 sequence isolated the difference between a compilable iterator alias and a valid shared-memory contract. v310’s congruous probe returned invalid raw values; v311’s logical subbyte conversion returned zeros; v313’s enlarged backing extent still returned zeros; and v315’s `<K,8>` geometry still produced an illegal address. The source audit then confirmed that production `DefaultMmaCore` uses `RowMajorTensorOpMultiplicandCrosswise<4,64>` for A and `ColumnMajorTensorOpMultiplicandCrosswise<4,64>` for B, with separate MatrixShape contracts A=`<8,32>` and B=`<32,8>`.

v316 changes only the research-only aliases and probe setup: it uses CUTLASS’s production Crosswise row/column-major wrappers, separate A/B TensorRefs, exact A extent `<64,128>` and B extent `<128,64>`, and fills packed storage through each layout’s logical-to-physical mapping. Production dispatch remains disabled. The authoritative T4 run was Kaggle kernel version 314 and completed successfully. It reported `SM75_INT4_WARP_ITERATOR_PROBE_PASS [1, 1, 2]`, together with the previously passing native arithmetic, WMMA load, fused, and mixed-stride probes. Embedded tests returned 0; T4 hardware, native correctness, and timing gates passed. The complete benchmark still reported the existing mixed-prefill end-to-end and timing-integrity failures, and full-model Qwen quality/throughput plus vLLM apply remained unavailable, so the terminal decision was `no_go` and v299 remains the production baseline.

This is a meaningful research milestone: the next native block candidate can load U4/S4 operands using the same Crosswise shared-memory ABI as production rather than the rejected generic DefaultMmaCore or guessed Congruous layout. No production performance claim is made until a block-level decomposition kernel proves exact outputs, tails, scale/zero handling, indexed scatter, memory accounting, and end-to-end T4 gates.

## v317–v321 SM75 native INT4 fragment seam

The v317 submission (Kaggle kernel version 317) compiled and ran the loaded Crosswise pair-MMA experiment, but its result was `[1, 1, 2, 32, 32, 32, 32]` rather than the naïve `[1, 1, 2, 64, 64, 64, 64]`. This was not accepted as a pass; the result demonstrated that fragment/register ownership cannot be inferred from `FragmentC[0/1]` as scalar matrix outputs.

The v318 submission (Kaggle kernel version 318, source v320) exported all eight slots from each loaded A/B fragment and all four pair-MMA accumulators. The authoritative first-lane and all-lane-stable payload was `A_u4=[1,0,1,0,1,0,1,0]`, `A_s4=[1,0,1,0,1,0,1,0]`, `B_u4=[2,2,2,2,2,2,2,2]`, and accumulators `[32,32,32,32]`. The assertion failed against the intentionally dense expected fragment, so this is an explicit negative diagnostic, not a relaxed pass. It indicates that the LDSM b16 register words are being viewed through a dense 4-bit Array accessor and/or that the Crosswise backing storage does not match the native MMA register contract.

Source research against vendored CUTLASS established that the Crosswise wrappers reverse matrix-to-pitch dimensions and delegate to the SM75 Crosswise iterator. The iterator uses LDSM x4 and reinterprets the fragment as four unsigned 32-bit words. CUTLASS's `Array<T,N,false>` packs subbyte logical values, while LDSM moves 16-bit matrix words; therefore a per-lane fragment is not a continuous logical matrix slice. CUTLASS's own warp testbed instead stores the accumulator fragment through `MmaTensorOpAccumulatorTileIterator` and compares a complete output matrix to a reference. The official CUTLASS tile-iterator documentation and NVIDIA's CUTLASS issue #1635 reinforce that lane fragments are intentionally non-contiguous.

The v319 submission (Kaggle kernel version 317, commit `56afc91`) was not an independent success; it only adjusted the prior loaded-fragment experiment and failed the original iterator expectation. The v320 commit (`c8cd080`) exposed the full fragment payload and retained the mismatch as evidence. The v321 commit (`532ca81`) adds a research-only full 8x8 proof: three uniform low/high nibble cases are loaded via WMMA packed shared tiles, both native SM75 MMAs are invoked, their combined result is stored through CUTLASS's row-major accumulator iterator, and the full 8x8 output is checked. Local source tests remain green (`94 passed, 6 skipped, 3 subtests passed`), but v321 has not yet been T4-validated because the temporary Kaggle credentials were removed after the required cleanup. Production remains the safe v299 expanded-INT4 plus staged-CUTLASS control; native routing is still disabled.

## v322–v323 source-only refinement — b16 physical Crosswise capture

v322 made the existing Crosswise diagnostic storage-safe: the iterator fragment capture now uses a 16-byte / 32-logical-element shape because SM75 LDSM x4 writes four 32-bit registers even though the native U4/S4 MMA operand ABI is one 32-bit register containing eight nibbles. The prior dense-u4 result remains an explicit expected-negative diagnostic rather than a pass.

Fresh CUTLASS source analysis then isolated the physical transfer granularity. The SM75 layout constants for ElementSize=4 give 32 logical elements per 128-bit access, while `ldmatrix.sync.aligned.x4...b16` transfers eight 16-bit words per lane. v323 adds a separate research-only `NativeWarpU16A/BIterator` contract using the same production Crosswise row/column swizzles with ElementSize=16. It allocates 16-bit shared storage, loads all logical values through the iterator, masks/repackages the low nibble into the native U4/S4 fragments, and reports the loaded slots and pair-MMA accumulators. The gate expects eight A values of 1, eight signed-A values of 1, eight B values of 2, and four accumulators of 64. This is a source-backed experiment, not a production change; local validation remains 94 passed, 6 skipped, 3 subtests passed, but v323 has not yet run on T4. The latest authoritative production baseline remains v299 with native dispatch disabled.

## v324–v326 — direct Crosswise b16 loader proof hardening

v324 extended the direct Crosswise b16 probe to store a complete 8x8 combined output through `MmaTensorOpAccumulatorTileIterator`, rather than checking only lane-local accumulator slots. v325 corrected the high plane to use a separate physical tile containing nibble `0xF` and explicitly sign-extended it to `-1` before constructing `int4b_t`; the expected combined uniform result is `(1 + 16*(-1)) * 2 * 32 = -960`. v326 fixed a probe-output indexing defect found by source audit: the 28-slot lane diagnostics use a 92-column row stride, leaving the appended 64-element matrix region non-overlapping. The source test and generated notebook pass locally after the correction.

The v326 candidate is committed as `0d4de49` and pushed to `unified-three-level-sm75`. It is still research-only and has not been submitted to T4 because the temporary credentials were removed as required. No production dispatch, quality gate, memory accounting, indexed-scatter ABI, or benchmark behavior was changed; v299 remains the production baseline.

## v327 T4 result — accumulator-store extension still terminates nvcc with code 255

The v327 source (Kaggle kernel version 320) corrected both accumulator-iterator instruction arguments from `MatrixShape<8,8>` to `gemm::GemmShape<8,8,32>`, addressing the explicit v326 `kM/kN/kMN` compile errors. The T4 run still failed during the CUDA extension build: Ninja reported nvcc exit code 255 with no additional CUDA error text, only unrelated unused-variable/function warnings. No probe, correctness, performance, quality, memory, or production result is accepted.

A bounded source reduction isolated the remaining risk to having two independent `MmaTensorOpAccumulatorTileIterator` instantiations in the same translation unit, one in the WMMA/native full-matrix proof and one in the direct Crosswise b16 probe. v328 removes only the second accumulator-store instantiation from the direct probe, retaining its direct U16 Crosswise load, explicit signed high-nibble conversion, and pair-MMA accumulator checks; the separate full-matrix WMMA/native-store proof remains. The direct probe output stride is restored to its safe 28-slot shape. Production dispatch and all v299 gates remain unchanged. Local validation after v328 remains 94 passed, 6 skipped, 3 subtests passed; v328 requires the next T4 run to determine whether the code-255 termination is resolved.

## v329–v334 — complete WMMA store seam and embedded CUTLASS dependency diagnosis

v329 (Kaggle kernel version 322) was rejected before CUDA template analysis because the newly referenced complete `DefaultMmaTensorOpWmma` selector header was absent from the embedded CUTLASS archive: `cutlass/gemm/warp/default_mma_wmma_tensor_op.h` was not found. This was a packaging/manifest defect, not evidence about the SM75 fragment contract. v330 added that header to `scripts/cutlass_sm75_sources.txt`, rebuilt `cutlass_sm75_vendor.b64`, and regenerated the notebook.

v330 (Kaggle kernel version 323) progressed one include deeper and failed because `default_mma_wmma_tensor_op.h` required the absent `cutlass/gemm/warp/mma_tensor_op_wmma.h`. v331 added this dependency to the manifest and rebuilt the archive. v331 (Kaggle kernel version 324) then progressed again and failed because `mma_tensor_op_wmma.h` required the absent `cutlass/gemm/warp/mma_tensor_op_tile_iterator_wmma.h`. v332 added that third dependency and preserved the research-only status; these three failures are now classified as incomplete embedded-vendor closure rather than algorithmic failures.

v332 (Kaggle kernel version 325) compiled far enough to instantiate the complete WMMA path and exposed the real type-contract issues. The selected operands were `cutlass::uint4b_t`, but vendored `cutlass/arch/wmma_sm75.h` provides the complete WMMA specialization for `cutlass::int4b_t`/S4, not U4. The same run also showed that CUDA’s `nvcuda::wmma::fragment<accumulator,...>` does not support the CUTLASS-style `combined[0]` assignment; the compiler reported a fragment with two underlying registers and no scalar subscript assignment. No probe result is accepted from this run.

v333 (Kaggle kernel version 326) changed only the complete WMMA selector operands from U4 to the source-supported S4 type. The T4 compiler accepted the dependency closure and reached the remaining fragment-access error: `combined[0]` and `combined[1]` were invalid assignments to `nvcuda::wmma::fragment`; nvcc reported exactly two errors at those lines. This confirms the selector is now instantiated and isolates the next correction to the native WMMA fragment register interface.

v334 changes those two writes to `combined.x[0]` and `combined.x[1]`, the CUDA WMMA fragment register-array interface. Local validation remains green: 94 passed, 6 skipped, 3 subtests passed; compileall, diff checks, and deterministic notebook regeneration pass. The change was committed as `ea30753` and pushed to `unified-three-level-sm75`, then submitted as Kaggle kernel version 327. The v334 job entered `QUEUED`; the bounded poll timed out and the Windows sidecar disconnected before terminal log retrieval, so v334 has no accepted T4 result yet. The temporary polling/fetch helpers were removed after the prior terminal runs; no duplicate submission was made.

All v329–v334 work remains research-only. Production routing remains the safe v299 expanded-INT4 plus staged-CUTLASS control, native dispatch is disabled, and no quality, memory, correctness, indexed-scatter ABI, model, vLLM, or end-to-end speed claim has changed. The complete WMMA selector path is being used only to validate accumulator reconstruction and must not be promoted until the full loader, signed/unsigned decomposition, tails, scale/zero correction, scatter, memory, model-quality, vLLM, and >=2.6x end-to-end gates pass.

Source comparison note: the decisive original/CUTLASS lesson in this iteration is that the complete store path is not interchangeable with the custom U4/S4 operand path. CUTLASS’s SM75 WMMA specialization is S4-oriented and owns the accumulator fragment through CUDA’s opaque `.x[]` register array, while the production native replacement still needs the Crosswise b16 loader and explicit U4/S4 two-MMA decomposition. The complete WMMA selector therefore remains a correctness oracle for matrix ownership, not a production implementation shortcut.

## Current state after v334

The current branch contains the v334 `.x[]` correction plus the previously embedded WMMA headers. The latest local commit is `ea30753`; GitHub branch `unified-three-level-sm75` is synchronized. The authoritative unresolved item is the terminal T4 result for Kaggle version 327. v299 remains the production baseline.

*End of recorded evidence.*

## v335–v338 — native SM75 accumulator-store proof reaches a positive T4 seam

v335 (Kaggle kernel version 328) compiled the complete research extension and all 95 embedded tests passed (`OK (skipped=2)`). Its direct physical Crosswise U16 loader/repack probe passed with `SM75_INT4_CROSSWISE_U16_PROBE_PASS [1,1,1,1,1,1,1,1,15,15,15,15,15,15,15,15,2,2,2,2,2,2,2,2,64,64,-64,-64]`. The three-case full-matrix proof also compiled and reached execution, but the WMMA/native accumulator handoff produced row 0 of 64 and rows 1–2 of `16843009` (`0x01010101`) instead of the required 1024 and -960. This was retained as a correctness failure: direct WMMA storage is not an ABI-compatible sink for manually combined native `arch::Mma` accumulator registers. Expected values were not relaxed.

v336 (Kaggle kernel version 329) replaced the WMMA fragment store call with direct `nvcuda::wmma::store_matrix_sync`, while preserving the same manually combined native U4/U4 and S4/U4 accumulator values. The extension and all 95 embedded tests again passed, and the direct Crosswise U16 probe remained positive, but the full-matrix payload was unchanged: row 0 was 64 and rows 1–2 were `16843009`, versus expected rows 64, 1024, and -960. This conclusively rejected the complete WMMA/native accumulator cross-ABI route as a production seam; the complete WMMA machinery remains only an ownership/reference oracle.

v337 (Kaggle kernel version 330, commit `cd8aa9c`) replaced the failed WMMA store handoff with CUTLASS’s native `MmaTensorOpAccumulatorTileIterator<MatrixShape<8,8>, int, RowMajor, InstructionShape, MatrixShape<1,1>>`, consuming the same two-int `LowMma::FragmentC` produced by the native pair MMA. The iterator instantiated and the T4 extension compiled; the direct Crosswise U16 probe and all earlier probes passed. The full proof nevertheless failed its exact matrix assertion with row 0 equal to 64 and rows 1–2 equal to `16843009`. Source inspection identified the defect in the proof harness rather than the iterator contract: the kernel was launched as three separate one-warp blocks (`<<<3,32>>>`), while `case_id = threadIdx.x / 32` is zero in every block and the per-block shared accumulator is therefore only initialized/stored for case 0. Cases 1 and 2 were uninitialized output, not evidence against the native iterator. No expected value or gate was changed.

v338 (Kaggle kernel version 331, commit `ceb41fb`) corrected only the research-harness launch to one block containing three case warps (`<<<1, 3 * kWarpSize>>>`), matching the existing `case_id` calculation and shared-memory case partition. Local validation passed: 94 passed, 6 skipped, 3 subtests passed; compileall, `git diff --check`, deterministic notebook generation, and notebook `--check` passed. The authoritative NVIDIA T4 run completed with embedded tests returning 0 and reported `SM75_INT4_CROSSWISE_U16_PROBE_PASS`, `SM75_INT4_WMMA_NATIVE_STORE_PROBE_PASS [[64,...],[1024,...],[-960,...]]`, the WMMA-load probe pass, fused probe pass, mixed-stride mismatch count 0, and `TESTS_RETURNCODE 0`. This is the first positive end-to-end research proof of the combined seam: production-style physical Crosswise b16/U16 loading, explicit U4/S4 nibble repacking and sign extension, two native SM75 pair MMAs, and a native CUTLASS accumulator iterator store reconstructing all three exact 8x8 matrices. It is still not a production implementation: the full proof uses uniform synthetic cases, does not yet cover nonuniform values, tile/K offsets, tails, scale/zero correction, indexed scatter, full model quality, vLLM execution, or honest end-to-end speed. The gate correctly remains `no_go` for production; the safe v299 expanded-INT4 plus staged-CUTLASS control remains authoritative, with `native_v3=None` and `use_cached_v3=False` unchanged.

The v338 benchmark continued to show the existing production limitations rather than a new performance claim: `sm75_native_correctness` and `timing_integrity` passed, while mixed decode/prefill end-to-end performance and `operator_production` failed; full-model Qwen quality/throughput and vLLM apply were unavailable in the environment. For the measured mixed Qwen rows=128 scenario, the existing safe route reported 0.7657 ms end-to-end versus 0.2458 ms dense FP16 (3.12x latency ratio), and pure INT4 reported 0.2302 ms versus 0.1558 ms (1.48x ratio). These measurements are not attributable to the research-only native aliases and do not satisfy the mandatory >=2.6x honest FP16 E2E target.

## Current state after v338

The branch is synchronized at `ceb41fb v338_fix_three_case_probe_launch`. The native accumulator iterator seam is now positive on authoritative T4 evidence for the limited uniform 8x8 proof. The next bounded research step is to extend that proof with nonuniform U4/S4 values, zeros and negative signed nibbles, independent A/B patterns, and a direct full-matrix integer reference, followed by K/tile-offset and tail cases. Production remains v299 until every correctness, ABI, memory, model-quality, vLLM, timing-integrity, benchmark, scatter, and >=2.6x E2E gate passes.

*End of recorded evidence.*

## v339–v340 — nonuniform reference proof and exact-reference portability fix

v339 (Kaggle kernel version 332) retained the corrected four-case native-store launch and added a deterministic nonuniform case with varied U4 low nibbles, signed S4 high nibbles, and varied U4 weights. The CUDA extension compiled and the physical Crosswise U16 loader probe passed, but the notebook harness failed before comparing the native result because this Kaggle PyTorch build does not implement CUDA `int32` matrix multiplication: `NotImplementedError: "addmm_cuda" not implemented for 'Int'`. This is a gate-harness portability defect, not evidence against the native loader/MMA/store contract. The expected matrix was not altered to fit any observed output.

v340 (Kaggle kernel version 333, commit `050f18f`) changed only the reference calculation to exact CPU int64 matrix multiplication, cast the result to int32, and transferred it to the T4 for comparison. The authoritative run completed successfully: all 95 embedded tests returned `OK (skipped=2)`, the Crosswise U16 probe passed, and `SM75_INT4_WMMA_NATIVE_STORE_PROBE_PASS` reported the exact four rows `[[64,...],[1024,...],[-960,...],[-3360,-304,544,720,-1312,-4528,-1248,336]]`. The independent-reference marker reported the same first eight nonuniform outputs: `SM75_INT4_WMMA_NATIVE_STORE_NONUNIFORM_REFERENCE_PASS [-3360,-304,544,720,-1312,-4528,-1248,336]`. This validates varied low values, signed high values including negative int4 codes, zero-valued contributions, varied U4 B values, and full 8x8 native accumulator storage for one K=32 tile.

v340 still correctly ended `terminal_decision: no_go`: `operator_production` and `model_vllm_production` remained failed, full-model quality/throughput and vLLM apply were unavailable, and mixed prefill end-to-end performance remained failed. The research-only proof does not change production routing, memory accounting, indexed scatter, timing integrity, or the mandatory >=2.6x honest FP16 E2E requirement. The safe v299 expanded-INT4 plus staged-CUTLASS control remains authoritative.

## v341 T4 result — physical Crosswise U16 loader to native full-matrix store passes

v341 (Kaggle kernel version 334, commit `01ddea8`) added a separate research-only operator that fills the established large Crosswise physical backing tiles, loads A and B through `NativeWarpU16AIterator`/`NativeWarpS16AIterator`/`NativeWarpU16BIterator`, repacks all eight lane-local 16-bit values into the native U4/S4 operand fragments, invokes the two native m8n8k32 MMAs, and stores through the shared CUTLASS `MmaTensorOpAccumulatorTileIterator` helper. The gate’s exact reference is the same deterministic CPU int64 calculation used for the WMMA-loaded nonuniform proof.

The authoritative NVIDIA T4 run compiled successfully and returned `TESTS_RETURNCODE 0`. It reported the existing positive probes plus `SM75_INT4_CROSSWISE_U16_NATIVE_STORE_REFERENCE_PASS [[-3360,-304,544,720],[-4528,-1248,336,736]]`, matching the full 8x8 reference matrix. The underlying uniform and nonuniform WMMA/native proof also passed, as did the Crosswise U16 loader, fused, and mixed-stride probes. This is the strongest research result so far: varied physical Crosswise b16/U16 words, U4 low-plane extraction, signed S4 high-plane extraction, native pair MMA arithmetic, and native CUTLASS accumulator storage agree with an independent full-matrix reference on T4.

The result remains a limited K=32 origin-tile proof. It does not yet validate a second K tile and accumulation across K, iterator advancement or physical tile offsets, partial K tails, quantization scale/zero correction, indexed scatter, full production block scheduling, model quality, vLLM execution, or the >=2.6x honest FP16 E2E target. The full gate correctly remains `terminal_decision: no_go`; `operator_production` and `model_vllm_production` are failed, full-model Qwen quality/throughput and vLLM apply are unavailable, and mixed prefill E2E remains failed. Production stays on v299 with native routing disabled.

## v342 T4 result — Crosswise iterator increment and K64 accumulation pass

v342 (Kaggle kernel version 335, commit `7ebedaf`) added a separate research-only K64 operator. It initializes the established physical Crosswise U16 backing layouts, loads two consecutive K=32 tiles, advances all three U16 iterators with CUTLASS `operator++`, repacks each tile into native U4/S4 fragments, accumulates both native MMAs into one `FragmentC`, combines the low and high planes, and stores once through the shared native accumulator iterator. The gate computes the K=64 reference independently on CPU with int64 arithmetic.

The authoritative T4 run compiled and passed all embedded checks (`TESTS_RETURNCODE 0`). It reported `SM75_INT4_CROSSWISE_U16_K64_NATIVE_STORE_REFERENCE_PASS [[-6720,-608,1088,1440],[-9056,-2496,672,1472]]`, exactly matching the independent 8x8 reference. Existing K=32 physical-loader, K=32 full-matrix nonuniform, WMMA/native store, expected-negative dense-u4, fused, and mixed-stride probes also passed. This validates the physical Crosswise U16 loader, legal U4/S4 repack, native pair MMA arithmetic, accumulator ABI, CUTLASS iterator increment across a second K tile, and accumulation across two m8n8k32 instructions on T4.

This remains a research-only operator proof, not a production implementation. The full run correctly remained `terminal_decision: no_go`: `operator_production` and `model_vllm_production` failed, full-model Qwen quality/throughput and vLLM apply were unavailable, mixed prefill E2E remained failed, and the reported `timing_integrity` pre-execution gate was failed because one measured mixed scenario had a ratio above its strict bound. No production route, v299 baseline, 4/8/16 partitioning, memory report, indexed scatter ABI, or performance claim was changed.

## v344 T4 result — distinct physical Crosswise M/N tile offsets pass

v344 (Kaggle kernel version 337, commit `67569ac`) corrected the prior M/N-offset harness so row and column blocks use non-aliasing deterministic formulas: A low=`(row + 5*k + 1) & 15`, A high raw=`(2*row + 3*k + 7) & 15`, and B=`(7*k + 5*col + 2) & 15`. The proof selects four independent outputs using CUTLASS Crosswise `add_tile_offset({1,0})` for the A row tile and `add_tile_offset({0,1})` for the B column tile, then compares all four matrices to an exact CPU int64 reference.

The authoritative T4 run compiled and passed all embedded checks (`TESTS_RETURNCODE 0`). It reported `SM75_INT4_CROSSWISE_U16_MN_OFFSET_NATIVE_STORE_REFERENCE_PASS` with four distinct matrices: case 0 A0/B0, case 1 A0/B1, case 2 A1/B0, and case 3 A1/B1. This closes the previously untested M/N physical Crosswise tile-offset seam in the research harness, in addition to the already positive K=32 origin, K64 iterator increment/accumulation, nonuniform signed/unsigned repack, native MMA, and native accumulator store proofs.

The run remains research-only and correctly ends `terminal_decision: no_go`; production operator and model/vLLM gates are still failed, full-model quality/throughput and vLLM apply are unavailable, and no production dispatch, memory accounting, indexed scatter ABI, timing claims, or >=2.6x E2E claim was changed. Production remains on the v299 expanded-INT4 plus staged-CUTLASS control with native routing disabled.

## v345 T4 result — four co-resident warps sharing physical Crosswise storage pass

v345 (Kaggle kernel version 338, commit `22d41b0`) added a research-only four-warp, 128-thread block proof. The block cooperatively initializes the same physical Crosswise U16 A/B backing tiles, synchronizes with `__syncthreads()`, assigns each warp one of the four distinct M/N tile combinations, applies CUTLASS Crosswise tile offsets, runs the native U4/U4 and S4/U4 pair, and stores each result into a disjoint 8x8 slice. The gate compares all four slices to the independent CPU int64 reference.

The authoritative T4 run compiled and passed all embedded checks (`TESTS_RETURNCODE 0`). It reported `SM75_INT4_CROSSWISE_U16_BLOCK_NATIVE_STORE_REFERENCE_PASS` with the same four distinct matrices as the one-warp offset proof. This validates shared-memory barrier visibility, co-resident warp ownership, physical Crosswise layout reuse across a block, per-warp M/N offsets, native pair MMA arithmetic, and disjoint native accumulator stores. Together with v342, the research seam now covers K64 iterator advancement and accumulation as well as block-level M/N distribution.

The run correctly remained `terminal_decision: no_go`; production operator/model-vLLM gates were still failed, full-model quality/throughput and vLLM apply were unavailable, and no production dispatch, 4/8/16 partitioning, memory accounting, indexed scatter ABI, timing claim, or >=2.6x honest FP16 E2E claim was changed. The v299 expanded-INT4 plus staged-CUTLASS control remains production baseline.
