    "mixllm_three_level",
    from .mixllm_three_level import MixLLMThreeLevelConfig
        "mixllm_three_level": MixLLMThreeLevelConfig,
...
class MixLLMThreeLevelLinearMethod(LinearMethodBase):
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor,
        ...
        weight = torch.empty((output_width, input_width), dtype=torch.float32,
                             device=x.device)
        group_size = self.quant_config.contract.group_size
        if layer.indices_16.numel():
            weight.index_copy_(0, layer.indices_16.long(), layer.weight_fp16.float())
        if layer.indices_8.numel():
            scales = layer.scale_int8.float().repeat_interleave(group_size, dim=1)
            weight.index_copy_(0, layer.indices_8.long(),
                               layer.weight_int8.float() * scales)
        if layer.indices_4.numel():
            codes = self._unpack_int4(layer.weight_int4).float()
            scales = layer.scale_int4.float().repeat_interleave(group_size, dim=1)
            zeros = layer.zero_int4.float().repeat_interleave(group_size, dim=1)
            weight.index_copy_(0, layer.indices_4.long(), (codes - zeros) * scales)
        return F.linear(x.float(), weight, None if bias is None else bias.float()).to(x.dtype)
