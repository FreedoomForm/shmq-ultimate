...
__global__ void three_level_decode_kernel(
  ...
  float result = 0.0f;
  __shared__ __align__(16) int8_t shared_input[kGroupSize];
  int output_channel;
  if (channel < n4) {
    ...
    for (int group = 0; group < groups; ++group) {
      ...
      for (int linear = threadIdx.x; linear < kGroupSize; linear += blockDim.x) {
        shared_input[linear] = input_int8[base + linear];
      }
      __syncthreads();
...
#pragma unroll
      for (int offset = sublane * 4; offset < kGroupSize;
        ...
        const int activations = *reinterpret_cast<const int*>(shared_input + offset);
