...
EXPECTED_COMMIT = "f8fe0f1"
ROOT = Path("/content/shmq-ultimate-v301")
...
def main() -> int:
    ...
    if notebook_override:
        ...
        if not commit.startswith(EXPECTED_COMMIT):
            raise RuntimeError(f"expected v301 commit prefix {EXPECTED_COMMIT}, got {commit}")
    ...
    if os.environ.get("SHMQ_OPERATOR_ONLY") == "1":
        ...
        prepare_qwen_model()
    colab_notebook = Path("/content/mixllm_3level_gate_colab_v301.ipynb")
