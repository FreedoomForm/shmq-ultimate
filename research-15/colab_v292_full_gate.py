...
EXPECTED_COMMIT = "9538853"
ROOT = Path("/content/shmq-ultimate-v302")
...
def main() -> int:
    ...
    if notebook_override:
        ...
        if not commit.startswith(EXPECTED_COMMIT):
            raise RuntimeError(f"expected v302 commit prefix {EXPECTED_COMMIT}, got {commit}")
    ...
    if os.environ.get("SHMQ_OPERATOR_ONLY") == "1":
        ...
        prepare_qwen_model()
    colab_notebook = Path("/content/mixllm_3level_gate_colab_v302.ipynb")
