...
EXPECTED_COMMIT = "032286f"
ROOT = Path("/content/shmq-ultimate-v298")
...
def main() -> int:
    ...
    if notebook_override:
        ...
        if not commit.startswith(EXPECTED_COMMIT):
            raise RuntimeError(f"expected v298 commit prefix {EXPECTED_COMMIT}, got {commit}")
    ...
    if os.environ.get("SHMQ_OPERATOR_ONLY") == "1":
        ...
        prepare_qwen_model()
    colab_notebook = Path("/content/mixllm_3level_gate_colab_v298.ipynb")
