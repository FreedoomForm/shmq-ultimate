...
def _use_v188_mixed_prefill_path(module, x, torch_module):
    """Disable the stale v188 mixed adapter after the cached-v3 retest.

    The function remains as an explicit dispatch seam and compatibility marker,
    but current mixed rows>=32 calls use the prepared CUTLASS metadata path.
    Keeping the old v2 entry point below preserves the ABI fallback for shapes
    that do not satisfy the cached-v3 dispatch conditions.
    """
    del module, x, torch_module
    return False
...
def three_level_linear_prequantized(
    ...
    native_v3 = None
    use_cached_v3 = False
    # The v3 ABI is the original MixLLM packed contract: raw weight_int4 for
    # decode/direct-pair compatibility, original-equivalent interleaved INT4
    ...
    if x.shape[0] >= 32:
        ...
        if native_v3 is not None:
            prefill_int4 = module.prepare_sm75_prefill_int4()
            metadata = _prefill_metadata_for_cutlass(module, x, torch_module)
            use_cached_v3 = prefill_int4 is not None and metadata is not None
