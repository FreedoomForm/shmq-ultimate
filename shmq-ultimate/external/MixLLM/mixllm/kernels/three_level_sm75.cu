...
__global__ void sm75_int4_pair_crosswise_u16_probe_kernel(int* output) {
#if __CUDA_ARCH__ >= 750
  ...
  for (int logical = lane; logical < 128 * 64; logical += kWarpSize) {
    ...
    b_storage[static_cast<int>(b_layout(coord))] = 2;
  }
  __syncwarp();
  ...
  U16BIterator::Fragment u16b_fragment;
  u16a.load(u16a_fragment);
  s16a.load(s16a_fragment);
  u16b.load(u16b_fragment);
...
at::Tensor sm75_int4_pair_crosswise_u16_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
...
using NativeAccumulatorIterator =
    cutlass::gemm::warp::MmaTensorOpAccumulatorTileIterator<
        cutlass::MatrixShape<8, 8>, int, cutlass::layout::RowMajor,
        shmq_cutlass_sm75::int4_pair_probe::InstructionShape,
        cutlass::MatrixShape<1, 1>>;

__device__ inline void store_native_accumulator(
    int* output, int lane,
    shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentC const& combined) {
  NativeAccumulatorIterator iter_c(
      {output, cutlass::layout::RowMajor::packed({8, 8})}, lane);
...
__global__ void sm75_int4_pair_wmma_native_store_probe_kernel(int* output) {
  ...
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentC combined;
  combined.clear();
  combined[0] = low_accum[0] + 16 * high_accum[0];
  combined[1] = low_accum[1] + 16 * high_accum[1];
  store_native_accumulator(
      accumulator + case_id * kMatrixElements, lane, combined);
  __syncwarp();
...
__global__ void sm75_int4_pair_crosswise_u16_native_store_probe_kernel(
    int* output) {
#if __CUDA_ARCH__ >= 750
  using U16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpU16AIterator;
  using S16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpS16AIterator;
  ...
  __syncwarp();
  for (int logical = lane; logical < 8 * 32; logical += kWarpSize) {
    ...
    a_high_storage[static_cast<int>(a_layout(coord))] =
        static_cast<uint16_t>(high);
  }
  for (int logical = lane; logical < 32 * 8; logical += kWarpSize) {
    const int k = logical / 8;
    const int col = logical % 8;
  ...
  combined[0] = low_accum[0] + 16 * high_accum[0];
  combined[1] = low_accum[1] + 16 * high_accum[1];
  store_native_accumulator(output, lane, combined);
#endif
}

at::Tensor sm75_int4_pair_crosswise_u16_native_store_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
              "SM75 Crosswise U16 native-store probe requires a CUDA tensor argument");
  auto output = at::empty({8, 8}, device_tensor.options().dtype(at::kInt));
  auto stream = at::cuda::getCurrentCUDAStream();
  sm75_int4_pair_crosswise_u16_native_store_probe_kernel<<<1, kWarpSize, 0, stream>>>(
      output.data_ptr<int>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return output;
}

template <typename AIterator, typename SIterator, typename BIterator>
__device__ inline void load_pair_store_native_matrix(
    AIterator& a_iterator, SIterator& s_iterator, BIterator& b_iterator,
    int* output, int lane) {
  typename AIterator::Fragment a_fragment;
  typename SIterator::Fragment s_fragment;
  typename BIterator::Fragment b_fragment;
  a_iterator.load(a_fragment);
  s_iterator.load(s_fragment);
  b_iterator.load(b_fragment);
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentA low_a;
  shmq_cutlass_sm75::int4_pair_probe::HighMma::FragmentA high_a;
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentB weights;
  low_a.clear();
  ...
  weights.clear();
  for (int i = 0; i < 8; ++i) {
    low_a[i] = cutlass::uint4b_t(static_cast<unsigned>(a_fragment[i]) & 0xf);
    int high_value = static_cast<int>(s_fragment[i]) & 0xf;
    high_value = high_value >= 8 ? high_value - 16 : high_value;
    high_a[i] = cutlass::int4b_t(high_value);
    weights[i] = cutlass::uint4b_t(static_cast<unsigned>(b_fragment[i]) & 0xf);
  }
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentC low_accum;
  shmq_cutlass_sm75::int4_pair_probe::HighMma::FragmentC high_accum;
  low_accum.clear();
...
__global__ void sm75_int4_pair_crosswise_u16_k64_native_store_probe_kernel(
    int* output) {
#if __CUDA_ARCH__ >= 750
  using U16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpU16AIterator;
  using S16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpS16AIterator;
  ...
  __syncwarp();
  for (int logical = lane; logical < 8 * 64; logical += kWarpSize) {
    const int row = logical / 64;
    const int k = logical % 64;
    const int low = (row * 3 + k * 5 + 1) & 15;
    const int high = (row * 5 + k * 3 + 7) & 15;
    const U16AIterator::TensorCoord coord(row, k);
    a_low_storage[static_cast<int>(a_layout(coord))] =
  ...
  for (int logical = lane; logical < 64 * 8; logical += kWarpSize) {
    const int k = logical / 8;
    const int col = logical % 8;
    const int weight = (k * 7 + col * 3 + 2) & 15;
    const U16BIterator::TensorCoord coord(k, col);
  ...
  S16AIterator s16a(S16AIterator::TensorRef(a_high_storage, a_layout), lane);
  U16BIterator u16b(U16BIterator::TensorRef(b_storage, b_layout), lane);
  U16AIterator::Fragment u16a_fragment0;
  S16AIterator::Fragment s16a_fragment0;
  U16BIterator::Fragment u16b_fragment0;
  u16a.load(u16a_fragment0);
  s16a.load(s16a_fragment0);
  u16b.load(u16b_fragment0);
  ++u16a;
  ++s16a;
  ++u16b;
  U16AIterator::Fragment u16a_fragment1;
  S16AIterator::Fragment s16a_fragment1;
  U16BIterator::Fragment u16b_fragment1;
  u16a.load(u16a_fragment1);
  s16a.load(s16a_fragment1);
  u16b.load(u16b_fragment1);

  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentA low_a0;
  shmq_cutlass_sm75::int4_pair_probe::HighMma::FragmentA high_a0;
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentB weights0;
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentA low_a1;
  shmq_cutlass_sm75::int4_pair_probe::HighMma::FragmentA high_a1;
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentB weights1;
  low_a0.clear();
  high_a0.clear();
  weights0.clear();
  low_a1.clear();
  ...
  weights1.clear();
  for (int i = 0; i < 8; ++i) {
    low_a0[i] = cutlass::uint4b_t(static_cast<unsigned>(u16a_fragment0[i]) & 0xf);
    int high0 = static_cast<int>(s16a_fragment0[i]) & 0xf;
    high0 = high0 >= 8 ? high0 - 16 : high0;
    high_a0[i] = cutlass::int4b_t(high0);
    weights0[i] = cutlass::uint4b_t(static_cast<unsigned>(u16b_fragment0[i]) & 0xf);
    low_a1[i] = cutlass::uint4b_t(static_cast<unsigned>(u16a_fragment1[i]) & 0xf);
    int high1 = static_cast<int>(s16a_fragment1[i]) & 0xf;
    high1 = high1 >= 8 ? high1 - 16 : high1;
    high_a1[i] = cutlass::int4b_t(high1);
    weights1[i] = cutlass::uint4b_t(static_cast<unsigned>(u16b_fragment1[i]) & 0xf);
  }
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentC low_accum;
  shmq_cutlass_sm75::int4_pair_probe::HighMma::FragmentC high_accum;
  low_accum.clear();
  ...
  shmq_cutlass_sm75::int4_pair_probe::LowMma low_mma;
  shmq_cutlass_sm75::int4_pair_probe::HighMma high_mma;
  low_mma(low_accum, low_a0, weights0, low_accum);
  high_mma(high_accum, high_a0, weights0, high_accum);
  low_mma(low_accum, low_a1, weights1, low_accum);
  high_mma(high_accum, high_a1, weights1, high_accum);
  shmq_cutlass_sm75::int4_pair_probe::LowMma::FragmentC combined;
  combined.clear();
  combined[0] = low_accum[0] + 16 * high_accum[0];
  combined[1] = low_accum[1] + 16 * high_accum[1];
...
at::Tensor sm75_int4_pair_crosswise_u16_k64_native_store_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
              "SM75 Crosswise U16 K64 native-store probe requires a CUDA tensor argument");
  auto output = at::empty({8, 8}, device_tensor.options().dtype(at::kInt));
  auto stream = at::cuda::getCurrentCUDAStream();
  sm75_int4_pair_crosswise_u16_k64_native_store_probe_kernel<<<1, kWarpSize, 0, stream>>>(
      output.data_ptr<int>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return output;
}

__global__ void sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe_kernel(
    int* output) {
#if __CUDA_ARCH__ >= 750
  using U16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpU16AIterator;
  using S16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpS16AIterator;
  ...
  U16AIterator a0(U16AIterator::TensorRef(a_low_storage, a_layout), lane);
  S16AIterator s0(S16AIterator::TensorRef(a_high_storage, a_layout), lane);
  U16BIterator b0(U16BIterator::TensorRef(b_storage, b_layout), lane);
  U16AIterator a1 = a0;
  S16AIterator s1 = s0;
  U16BIterator b1 = b0;
  a1.add_tile_offset(U16AIterator::TensorCoord(1, 0));
  s1.add_tile_offset(S16AIterator::TensorCoord(1, 0));
  b1.add_tile_offset(U16BIterator::TensorCoord(0, 1));

  load_pair_store_native_matrix(a0, s0, b0, output + 0 * 64, lane);
  load_pair_store_native_matrix(a0, s0, b1, output + 1 * 64, lane);
  load_pair_store_native_matrix(a1, s1, b0, output + 2 * 64, lane);
  load_pair_store_native_matrix(a1, s1, b1, output + 3 * 64, lane);
#endif
}

at::Tensor sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
              "SM75 Crosswise U16 M/N-offset native-store probe requires a CUDA tensor argument");
  auto output = at::empty({4, 8, 8}, device_tensor.options().dtype(at::kInt));
  auto stream = at::cuda::getCurrentCUDAStream();
  sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe_kernel<<<1, kWarpSize, 0, stream>>>(
      output.data_ptr<int>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return output;
}

__global__ void sm75_int4_pair_crosswise_u16_block_native_store_probe_kernel(
    int* output) {
#if __CUDA_ARCH__ >= 750
  using U16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpU16AIterator;
  using S16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpS16AIterator;
  ...
  for (int logical = thread; logical < 16 * 32; logical += blockDim.x) {
    const int row = logical / 32;
    const int k = logical % 32;
    const int low = (row + k * 5 + 1) & 15;
    const int high = (row * 2 + k * 3 + 7) & 15;
  ...
  for (int logical = thread; logical < 32 * 16; logical += blockDim.x) {
    const int k = logical / 16;
    const int col = logical % 16;
    const int weight = (k * 7 + col * 5 + 2) & 15;
    const U16BIterator::TensorCoord coord(k, col);
  ...
  U16AIterator a_iterator(U16AIterator::TensorRef(a_low_storage, a_layout), lane);
  S16AIterator s_iterator(S16AIterator::TensorRef(a_high_storage, a_layout), lane);
  U16BIterator b_iterator(U16BIterator::TensorRef(b_storage, b_layout), lane);
  if (warp >= 2) {
    a_iterator.add_tile_offset(U16AIterator::TensorCoord(1, 0));
    s_iterator.add_tile_offset(S16AIterator::TensorCoord(1, 0));
  }
  if (warp & 1) {
    b_iterator.add_tile_offset(U16BIterator::TensorCoord(0, 1));
  }
  load_pair_store_native_matrix(a_iterator, s_iterator, b_iterator,
                                output + warp * 64, lane);
#endif
}

at::Tensor sm75_int4_pair_crosswise_u16_block_native_store_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
              "SM75 Crosswise U16 block native-store probe requires a CUDA tensor argument");
  auto output = at::empty({4, 8, 8}, device_tensor.options().dtype(at::kInt));
  auto stream = at::cuda::getCurrentCUDAStream();
  sm75_int4_pair_crosswise_u16_block_native_store_probe_kernel<<<1, 4 * kWarpSize, 0, stream>>>(
      output.data_ptr<int>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return output;
}

__global__ void sm75_int4_pair_packed_u4_block_native_store_probe_kernel(
    int* output) {
#if __CUDA_ARCH__ >= 750
  using U16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpU16AIterator;
  using S16AIterator = shmq_cutlass_sm75::int4_pair_probe::NativeWarpS16AIterator;
  ...
  for (int item = thread; item < 16 * 32; item += blockDim.x) {
    const int row = item / 32;
    const int k = item % 32;
    const int value = ((row * 29 + k * 17 + 3) % 127) - 63;
    activation_storage[item] = static_cast<int8_t>(value);
  }
  for (int item = thread; item < 16 * 16; item += blockDim.x) {
    const int channel = item / 16;
    const int byte = item % 16;
    const int even_code = (channel * 11 + (2 * byte) * 7 + 2) & 15;
    const int odd_code = (channel * 11 + (2 * byte + 1) * 7 + 2) & 15;
    packed_weight_storage[item] = static_cast<uint8_t>(even_code | (odd_code << 4));
  }
  __syncthreads();
  for (int item = thread; item < 16 * 32; item += blockDim.x) {
    const int row = item / 32;
    const int k = item % 32;
    const int value = static_cast<int>(activation_storage[item]);
    const int low = value & 15;
    const int high = value >> 4;
    const U16AIterator::TensorCoord coord(row, k);
    a_low_storage[static_cast<int>(a_layout(coord))] =
        static_cast<uint16_t>(low);
    a_high_storage[static_cast<int>(a_layout(coord))] =
        static_cast<uint16_t>(high & 15);
  }
  for (int item = thread; item < 32 * 16; item += blockDim.x) {
    const int k = item / 16;
    const int channel = item % 16;
    const uint8_t packed = packed_weight_storage[channel * 16 + k / 2];
    const int code = (k & 1) ? (packed >> 4) : (packed & 15);
    const U16BIterator::TensorCoord coord(k, channel);
    b_storage[static_cast<int>(b_layout(coord))] =
...
at::Tensor sm75_int4_pair_packed_u4_block_native_store_probe_cuda(
    const at::Tensor& device_tensor) {
  TORCH_CHECK(device_tensor.is_cuda(),
              "SM75 packed U4 block native-store probe requires a CUDA tensor argument");
  auto output = at::empty({4, 8, 8}, device_tensor.options().dtype(at::kInt));
  auto stream = at::cuda::getCurrentCUDAStream();
  sm75_int4_pair_packed_u4_block_native_store_probe_kernel<<<1, 4 * kWarpSize, 0, stream>>>(
      output.data_ptr<int>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return output;
}

at::Tensor sm75_int4_pair_wmma_load_probe_cuda(
const at::Tensor& device_tensor) {
...
TORCH_LIBRARY(mixllm_sm75, m) {
  ...
  m.def("sm75_int4_pair_crosswise_u16_probe(Tensor device_tensor) -> Tensor");
  m.def("sm75_int4_pair_crosswise_u16_native_store_probe(Tensor device_tensor) -> Tensor");
  m.def("sm75_int4_pair_crosswise_u16_k64_native_store_probe(Tensor device_tensor) -> Tensor");
  m.def("sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe(Tensor device_tensor) -> Tensor");
  m.def("sm75_int4_pair_crosswise_u16_block_native_store_probe(Tensor device_tensor) -> Tensor");
  m.def("sm75_int4_pair_packed_u4_block_native_store_probe(Tensor device_tensor) -> Tensor");
...
TORCH_LIBRARY_IMPL(mixllm_sm75, CUDA, m) {
  ...
  m.impl("sm75_int4_pair_crosswise_u16_probe", &sm75_int4_pair_crosswise_u16_probe_cuda);
  m.impl("sm75_int4_pair_crosswise_u16_native_store_probe", &sm75_int4_pair_crosswise_u16_native_store_probe_cuda);
  m.impl("sm75_int4_pair_crosswise_u16_k64_native_store_probe", &sm75_int4_pair_crosswise_u16_k64_native_store_probe_cuda);
  m.impl("sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe", &sm75_int4_pair_crosswise_u16_mn_offset_native_store_probe_cuda);
  m.impl("sm75_int4_pair_crosswise_u16_block_native_store_probe", &sm75_int4_pair_crosswise_u16_block_native_store_probe_cuda);
  m.impl("sm75_int4_pair_packed_u4_block_native_store_probe", &sm75_int4_pair_packed_u4_block_native_store_probe_cuda);
