...
$test = Join-Path $root 'shmq-ultimate\external\MixLLM\mixllm\test\test_sm75_source.py'
$backup = Join-Path $root 'research-15\audit_backups\test_sm75_source.before-v110.py'
New-Item -ItemType Directory -Force -Path (Split-Path $backup -Parent) | Out-Null
if (Test-Path -LiteralPath $test) { Copy-Item -LiteralPath $test -Destination $backup -Force }
$text = @'
...
class SM75SourceContractTest(unittest.TestCase):
    ...
    def test_m8n32_prefill_ownership_and_output_indexing_are_explicit(self):
        ...
        self.assertIn("(warp % 2) * 32", text)
        self.assertIn("channel_base = (tile_id - (precision == 4 ? 0 : tiles4)) * kPrefillChannels", text)
...
Write-Output ('LINES='+($text -split "`n").Count)
