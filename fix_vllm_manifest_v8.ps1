...
$manifest = Join-Path $root 'shmq-ultimate\external\MixLLM\vllm_v0.9.0_patch\THREE_LEVEL_MANIFEST.md'
$backup = Join-Path $root 'research-15\audit_backups\THREE_LEVEL_MANIFEST.before-v109.md'
...
Write-Output ('SM75_DOC='+$text.Contains('get_min_capability() is 75'))
Write-Output ('DIRECT_LAYOUT_DOC='+$text.Contains('direct output-channel layout'))
Write-Output ('SEPARATE_LAUNCH_DOC='+$text.Contains('separate activation-quantization'))
